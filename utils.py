...
def process_and_normalize(data, surf_type):
    ...
    """
    file_path = path + f'stats/stat_{surf_type}_detection_rev3.mat'
...
def load_models():
    ...
    models = {
        'ocean': {
            'dtc': pickle.load(open(path_models + 'ocean_det_xgb_rev4.pickle.dat', "rb")),
            'rain': keras.models.load_model(path_models + 'ocean_est_FL_rain_10class_rev2.keras', custom_objects={'CategoricalFocalLoss': CategoricalFocalLoss(alpha=0.25, gamma=2)}, compile=False),
            'snow': keras.models.load_model(path_models + 'ocean_est_FL_snow_10class_rev2.keras', custom_objects={'CategoricalFocalLoss': CategoricalFocalLoss(alpha=0.25, gamma=2)}, compile=False)
        },
        'land': {
            'dtc': pickle.load(open(path_models + 'land_det_xgb_rev2.pickle.dat', "rb")),
            'rain': keras.models.load_model(path_models + 'land_est_FL_rain_10class_rev2.keras', custom_objects={'CategoricalFocalLoss': CategoricalFocalLoss(alpha=0.25, gamma=2)}, compile=False),
            'snow': keras.models.load_model(path_models + 'land_est_FL_snow_10class_rev2.keras', custom_objects={'CategoricalFocalLoss': CategoricalFocalLoss(alpha=0.25, gamma=2)}, compile=False)
        },
        'coast': {
            'dtc': pickle.load(open(path_models + 'coast_det_xgb_rev2.pickle.dat', "rb")),
            'rain': keras.models.load_model(path_models + 'coast_est_FL_rain_10class_rev2.keras', custom_objects={'CategoricalFocalLoss': CategoricalFocalLoss(alpha=0.25, gamma=2)}, compile=False),
            'snow': keras.models.load_model(path_models + 'coast_est_FL_snow_10class_rev2.keras', custom_objects={'CategoricalFocalLoss': CategoricalFocalLoss(alpha=0.25, gamma=2)}, compile=False)
        },
        'snow': {
            'dtc': pickle.load(open(path_models + 'snowcover_det_xgb_rev2.pickle.dat', "rb")),
            'rain': keras.models.load_model(path_models + 'snowcover_est_FL_rain_10class_rev2.keras', custom_objects={'CategoricalFocalLoss': CategoricalFocalLoss(alpha=0.25, gamma=2)}, compile=False),
            'snow': keras.models.load_model(path_models + 'snowcover_est_FL_snow_10class_rev2.keras', custom_objects={'CategoricalFocalLoss': CategoricalFocalLoss(alpha=0.25, gamma=2)}, compile=False)
        },
        'ice': {
            'dtc': pickle.load(open(path_models + 'seaice_det_xgb_rev2.pickle.dat', "rb")),
            'rain': keras.models.load_model(path_models + 'seaice_est_FL_rain_10class_rev2.keras', custom_objects={'CategoricalFocalLoss': CategoricalFocalLoss(alpha=0.25, gamma=2)}, compile=False),
            'snow': keras.models.load_model(path_models + 'seaice_est_FL_snow_10class_rev2.keras', custom_objects={'CategoricalFocalLoss': CategoricalFocalLoss(alpha=0.25, gamma=2)}, compile=False)
        }
    }

    # Extract specific layers as models
    models['ocean']['rain'] = tf.keras.Model(models['ocean']['rain'].layers[0].input, models['ocean']['rain'].get_layer('fc_3').output)
    models['ocean']['snow'] = tf.keras.Model(models['ocean']['snow'].layers[0].input, models['ocean']['snow'].get_layer('fc_3').output)

    models['land']['rain'] = tf.keras.Model(models['land']['rain'].layers[0].input, models['land']['rain'].get_layer('fc_3').output)
    models['land']['snow'] = tf.keras.Model(models['land']['snow'].layers[0].input, models['land']['snow'].get_layer('fc_3').output)

    models['coast']['rain'] = tf.keras.Model(models['coast']['rain'].layers[0].input, models['coast']['rain'].get_layer('fc_3').output)
    models['coast']['snow'] = tf.keras.Model(models['coast']['snow'].layers[0].input, models['coast']['snow'].get_layer('fc_3').output)

    models['snow']['rain'] = tf.keras.Model(models['snow']['rain'].layers[0].input, models['snow']['rain'].get_layer('fc_3').output)
    models['snow']['snow'] = tf.keras.Model(models['snow']['snow'].layers[0].input, models['snow']['snow'].get_layer('fc_3').output)

    models['ice']['rain'] = tf.keras.Model(models['ice']['rain'].layers[0].input, models['ice']['rain'].get_layer('fc_3').output)
    models['ice']['snow'] = tf.keras.Model(models['ice']['snow'].layers[0].input, models['ice']['snow'].get_layer('fc_3').output)
...
def load_feature_data():
    ...
    path_features = os.path.join(path,'features/featuers_2')
    features_data = {
        'ocean': {
...
def load_stats():
    ...
    stats = {
        'ocean': sio.loadmat(path_stat + 'stat_ocean_detection_rev2.mat'),
        'land': sio.loadmat(path_stat + 'stat_land_detection_rev2.mat'),
        'coast': sio.loadmat(path_stat + 'stat_coast_detection_rev2.mat'),
        'snow': sio.loadmat(path_stat + 'stat_snowcover_detection_rev2.mat'),
        'ice': sio.loadmat(path_stat + 'stat_seaice_detection_rev2.mat')
    }
    return stats
